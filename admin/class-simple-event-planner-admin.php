...
class Simple_Event_Planner_Admin {
    ...
    public function enqueue_scripts() {
        ...
        if (is_admin()) {
            ...
            wp_register_script($this->simple_event_planner . '-jquery-google-map-api', 'http://maps.googleapis.com/maps/api/js?'. esc_attr($api_key) .'v=3.24&libraries=places', '', '3.24', TRUE);
            ...
            wp_register_script('wp-color-picker-alpha', plugin_dir_url(__FILE__) . 'js/wp-color-picker-alpha.min.js', array( 'wp-color-picker' ), '1.2.2', TRUE);
    ...
    public function admin_powered_by($text) {
        ...
        if (is_admin() && ( in_array($screen->id, apply_filters('sep_pages', $sep_pages)) )) {
            $text = '<a href="' . esc_url("http://www.presstigers.com/") . '" target="_blank"><img src="' . plugins_url('/images/powerByIcon.png',__FILE__) . '" alt="Powered by PressTigers"></a>';
        }
        return $text;
    }
}