...
function sep_get_the_event_start_date($post = NULL) {
    ...
    if (isset($start_date) && '' != $start_date && 'checked' != $event_date) {
        ...
        $start_date = date_format($start_date, $date_settings);
    } else {
        $start_date = '';
    }
...
function sep_get_the_event_end_date($post = NULL) {
    ...
    if (isset($end_date) && '' != $end_date && 'checked' <> $event_date) {
        ...
        $sep_event_options = get_option('sep_event_options');
        $date_settings = isset($sep_event_options['sep_date_format']) ? $sep_event_options['sep_date_format'] : 'F j, Y';
        $end_date = date_format($end_date, $date_settings);
    } else {
        $end_date = '';
    }
...
add_action('single_event_listing_start', 'sep_event_start_date', 30);
 ...
 * @since 1.1.0
 */
function sep_event_details() {
    get_simple_event_planner_template('single-event/event-details.php');
}

add_action('single_event_listing_end', 'sep_event_details', 20);
...
function sep_get_the_event_two_dates_diff($post = NULL) {
    ...
    $even_count_days = '';
    $days = ( 1 == $interval->format( '%d' ) ) ? esc_html__('day', 'simple-event-planner') : esc_html__('days', 'simple-event-planner');
    $hours = ( 1 == $interval->format( '%h' ) ) ? esc_html__('hour', 'simple-event-planner') :esc_html__('hours', 'simple-event-planner');
    
    if ( 0 == $interval->format('%d') &&  0 < $interval->format( '%h' ) ) {
        $even_count_days = ' (' . $interval->format( '%h' ) . ' ' . $hours . ')';
    } elseif ( 0 < $interval->format('%d') ) {
...
function is_sep_shortcode() {
    ...
    return ( isset($post->post_content) && (has_shortcode($post->post_content, 'event_listing') || has_shortcode($post->post_content, 'event_calendar')) );
}
